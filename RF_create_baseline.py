...
def normalize(spectra):
    ...
    norm_spectra = np.divide(spectra, max_per_spectrum)
    norm_spectra = norm_spectra[:,:,crop_start:crop_end]
    return norm_spectra

def load_dataset(path, var_name, param_path, labels):
...
class BaselineCreator:
    def __init__(self):
        ideal, ideal_params = load_dataset(ideal_path, ideal_var_name, ideal_parameter_path, labels)
        ...
        real, real_params = load_dataset(real_path, real_var_name, real_parameter_path, labels)
        if real_mag:
            real = np.sqrt(real[:,0,:]**2 + real[:,1,:]**2)
