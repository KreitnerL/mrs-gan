...
class LabeledMatSpectralDataset_Simplified(BaseDataset):
    def name(self):
        return 'LabeledMatSpectralDataset_Simplified'
    ...
    def initialize(self, opt):
        ...
        datadir = './dataset'#os.path.join(self.opt.save_dir, 'data')
        util.mkdir(datadir)
        ...
        if not self.opt.phase_data_path:
            ...
            for i in progressbar(range(len(self.A_paths)), "Loading patient data: ", 20):
                if 'simplified_dataset_parameters.mat' in self.A_paths[i]:
                    params = io.loadmat(self.A_paths[i])
                    # print(params.keys())
                elif 'simplified_dataset_magnitude.mat' in self.A_paths[i]:
                    mag = io.loadmat(self.A_paths[i])
                    # print(mag.keys())
                elif 'simplified_dataset_spectra.mat' in self.A_paths[i]:
            ...
            # print(cho.shape)
            parameters = np.empty([size[0],19,1])
            ...
            parameters[:,14,:] = base_scale[:,0,:]
            parameters[:,15,:] = base_scale[:,1,:]
            parameters[:,16,:] = base_scale[:,2,:]
            parameters[:,17,:] = base_scale[:,3,:]
            parameters[:,18,:] = base_scale[:,4,:]
            del cho, cre, naa, glx, ins, mac, lip, t2, base_scale, data, mag, params, spec
            ...
            length, specLength, d = spectra.shape
            spectra = np.asarray(spectra.transpose(0,2,1),dtype='float64')
            magnitude = np.asarray(magnitude.transpose(0,2,1),dtype='float64')
